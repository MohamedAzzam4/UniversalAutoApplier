...
def _load_fixture(page: Page, name: str) -> None:
...
def test_usercentrics_necessary_only(page: Page) -> None:
...
@pytest.mark.playwright
def test_generic_necessary_only(page: Page) -> None:
    ...
    assert result.result == "resolved"
    assert result.clicked is True
    assert "reject all" in result.clicked_text.lower() or "necessary" in result.clicked_text.lower()
    assert not page.locator("#onetrust-consent-sdk").is_visible()


@pytest.mark.playwright
def test_unknown_cmp_blocked(page: Page) -> None:
...
@pytest.mark.playwright
def test_application_consent_not_clicked(page: Page) -> None:
...
@pytest.mark.playwright
def test_pilot_defect_regression_old_vs_new(page: Page) -> None:
...
@pytest.mark.playwright
def test_unresolved_consent_blocks_form(page: Page) -> None:
    ...
    if result.result == "blocked":
        ...
        assert result.clicked is False or result.clicked is True  # either, but still blocked
        assert page.locator("#custom-cmp-xyz").is_visible()
...
@pytest.mark.playwright
def test_handler_never_clicks_dangerous_submit(page: Page) -> None:
