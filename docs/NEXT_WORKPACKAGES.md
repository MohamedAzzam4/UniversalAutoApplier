...
### V2-07B follow-up — Persisted worker state in the Review tab

The current Review tab still calls the legacy in-memory API and displays
generic defaults for a production-worker run, while the Submit panel's GET
status view reads the persisted snapshot. Track this mismatch for V2-07B;
the V2-02 local viewport review does not claim full dashboard consistency.
