...
def create_orchestration_run(
    ...
    *,
    ...
    row = OrchestrationRunRow(
        ...
        newly_eligible_ids_json=None,
        targeted_ids_json=None,
        processed_ids_json=None,
        remaining_ids_json=None,
        targeted_count=0,
        processed_count=0,
        remaining_count=0,
        pipeline_run_ids_json=None,
        pass_count=0,
        errors_json=[],
        started_at=_utcnow(),
        finished_at=None,
...
def orchestration_run_to_dict(row: OrchestrationRunRow | None) -> dict[str, Any]:
    ...
    return {
        ...
        "pipeline_run_id_initial": row.pipeline_run_id_initial,
        "pipeline_state_initial": row.pipeline_state_initial,
        "pipeline_run_id": row.pipeline_run_id,
        "pipeline_state": row.pipeline_state,
        "queue_hash_before": row.queue_hash_before,
        "queue_hash_after": row.queue_hash_after,
        "queue_mtime_ns_before": row.queue_mtime_ns_before,
        "queue_mtime_ns_after": row.queue_mtime_ns_after,
        "queue_published": row.queue_published,
        "newly_eligible_count": row.newly_eligible_count or 0,
        "newly_eligible_ids": list(row.newly_eligible_ids_json or []),
        "targeted_ids": list(row.targeted_ids_json or []),
        "processed_ids": list(row.processed_ids_json or []),
        "remaining_ids": list(row.remaining_ids_json or []),
        "targeted_count": row.targeted_count or 0,
        "processed_count": row.processed_count or 0,
        "remaining_count": row.remaining_count or 0,
        "pipeline_run_ids": list(row.pipeline_run_ids_json or []),
